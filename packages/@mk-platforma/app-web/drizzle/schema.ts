import { pgTable, uniqueIndex, index, foreignKey, integer, text, varchar, timestamp, serial, boolean, numeric, jsonb } from "drizzle-orm/pg-core"

import { sql } from "drizzle-orm"


export const imageToPost = pgTable("_ImageToPost", {
	a: integer("A").notNull().references(() => image.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	b: integer("B").notNull().references(() => post.id, { onDelete: "cascade", onUpdate: "cascade" } ),
},
(table) => {
	return {
...
export const comment = pgTable("Comment", {
	...
	authorId: integer("author_id").notNull().references(() => user.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	postId: integer("post_id").notNull().references(() => post.id, { onDelete: "restrict", onUpdate: "cascade" } ),
...
export const post = pgTable("Post", {
	...
	locationId: integer("location_id").references(() => location.id, { onDelete: "set null", onUpdate: "cascade" } ),
	authorId: integer("author_id").notNull().references(() => user.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	expertEndorsementId: integer("expertEndorsement_id").references(() => postExpertEndorsement.id, { onDelete: "set null", onUpdate: "cascade" } ),
	isDeleted: boolean("isDeleted").default(false).notNull(),
},
(table) => {
	return {
		expertEndorsementIdKey: uniqueIndex("Post_expertEndorsement_id_key").on(table.expertEndorsementId),
	}
});

export const categoryToPost = pgTable("_CategoryToPost", {
	a: integer("A").notNull().references(() => category.id, { onDelete: "cascade", onUpdate: "cascade" } ),
	b: integer("B").notNull().references(() => post.id, { onDelete: "cascade", onUpdate: "cascade" } ),
},
(table) => {
	return {
...
export const postExpertEndorsement = pgTable("Post_ExpertEndorsement", {
	id: serial("id").primaryKey().notNull(),
...
export const postExpertEndorsementSkill = pgTable("Post_ExpertEndorsement_skill", {
	id: serial("id").primaryKey().notNull(),
	label: varchar("label", { length: 64 }).notNull(),
	level: integer("level"),
	expertEndorsementId: integer("expertEndorsement_id").notNull().references(() => postExpertEndorsement.id, { onDelete: "restrict", onUpdate: "cascade" } ),
},
(table) => {
	return {
		expertEndorsementIdLabelKey: uniqueIndex("Post_ExpertEndorsement_skill_expertEndorsement_id_label_key").on(table.label, table.expertEndorsementId),
	}
});
