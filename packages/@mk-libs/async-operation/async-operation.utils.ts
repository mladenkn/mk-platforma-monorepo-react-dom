...
export function assertIsSuccessfull<TData>(
  op: AsyncOperation<TData>
): asserts op is AsyncOperationSuccess<TData> {
...
export function mapData<TData, TDataMapped>(
  op: AsyncOperation<TData>,
  mapper: (v: TData) => TDataMapped
): AsyncOperation<TDataMapped> {
  if (op.status === "SUCCESS") return { status: "SUCCESS", data: mapper(op.data) }
  else return op
}

export function mapQueryStatusToAsyncOpStatus(
