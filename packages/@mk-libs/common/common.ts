...
export function parsePropertyPathFromExpression<TObject, TReturnValue>(expr: (a: TObject) => TReturnValue){
  ...
  if (cutFromLeft.endsWith('}'))
    return cutFromLeft.slice(0, cutFromLeft.length - 1)
  else if (cutFromLeft.endsWith(';}'))
    return cutFromLeft.slice(0, cutFromLeft.length - 2)
...
export function shallowPick<TType, TKey extends keyof TType>(object: TType, ...keys: TKey[]): Pick<TType, TKey>{
  const ret: any = {};
  keys.forEach(key => {
    ret[key] = object[key];
  })
	...
	function changes(object: T, base: T) {
		return transform(object, function(result: any, value, key) {
			if (!isEqual(value, base[key])) {
				result[key] = (isObject(value) && isObject(base[key])) ? changes(value as any, (base as any)[key]) : value;
			}
		});
	}
...
export function objectEntriesDeep(obj: Record<string, any>) {
  ...
  for(const [key, value] of Object.entries(obj)) {
    ...
    if(typeof value === "object") {
      ...
      const subEntriesMapped: [string, any][] = subEntries.map(([subEntryKey, subEntryValue]) => [key + "." + subEntryKey, subEntryValue]);
